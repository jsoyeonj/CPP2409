...
public:
    MapLoader() : map(MAPY, vector<int>(MAPX, 0)) {}

    vector<vector<int>> loadMap(const string)
    {
        ifstream file("map.txt");

        if (!file)
        {
            throw runtime_error("맵 파일을 열수 없습니다:");
        }
        try
        { // 1202예외추가
            for (int i = 0; i < MAPY; i++)
            {
                for (int j = 0; j < MAPX; j++)
                {
                    ...
                    if (value < 0 || value > 4)
                    {
                        throw invalid_argument("잘못된 맵 타일 값: " + to_string(value));
                    }
...
int main()
{
    ...
    try
    {
        map = loader.loadMap("map.txt");
        cout << "맵 로딩 성공" << endl;
    }
    catch (const exception &e) // 1202예외추가: 맵파일 로딩실패처리
    {
        cerr << "맵 로딩 실패: " << e.what() << endl;
        return 1;
    }
